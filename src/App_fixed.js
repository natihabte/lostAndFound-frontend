...
// Navigation Component - OUTSIDE App
function Navigation({ currentPage, setCurrentPage, isLoggedIn, userRole, handleLogin, handleLogout, setShowAddModal }) {
  return (
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <div className="flex-shrink-0 flex items-center">
              ...
              <span className="ml-2 text-xl font-bold text-gray-900">Public Sector Lost & Found</span>
            </div>
...
function AddItemModal({ newItem, setNewItem, setShowAddModal, handleAddItem }) {
  return (
...
// HomePage Component
function HomePage({ items, searchTerm, setSearchTerm, selectedCategory, setSelectedCategory, selectedStatus, setSelectedStatus, isLoggedIn, userRole, handleClaimItem, handleDeleteItem }) {
...
function ProfilePage({ userRole, items }) {
  return (
...
function AdminDashboard({ items, users, handleDeleteItem, setSelectedItem }) {
  const stats = {
...
export default function App() {
  ...
  }, [newItem, items.length, userRole]);

  const handleClaimItem = useCallback((itemId) => {
    setItems(prev => prev.map(item => item.id === itemId ? { ...item, claimed: true } : item));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <footer className="mt-12 py-8 text-center text-sm text-gray-500">
        Built with ❤️ — Public Sector Lost & Found Management SaaS Platform
      </footer>
    </div>
  );
}
